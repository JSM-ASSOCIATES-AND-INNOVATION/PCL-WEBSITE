        ...
        {Object.entries(facilitiesData).map(([categoryKey, category], categoryIdx) => (
          <div key={categoryKey} className="mb-24">
            ...
            >
              ...
              <p className="text-gray-400 text-center md:text-left">{category.description}</p>
            </motion.div>
              ...
              {category.facilities.map((facility) => (
                <motion.div key={facility.id} variants={itemVariants} className="h-full">
                  <Link to={`/campus/facilities/${facility.id}`} className="block h-full group">
                    <div className="bg-[#0a0a0a] border border-white/5 rounded-2xl overflow-hidden h-full flex flex-col group-hover:border-[#FFBF00]/50 transition-all duration-300 relative group-hover:shadow-[0_0_30px_rgba(255,191,0,0.1)]">
                      <div className="h-48 relative overflow-hidden">
                        ...
                        <div className="absolute bottom-4 left-6 bg-[#111] border border-white/10 rounded-full w-12 h-12 flex items-center justify-center text-xl shadow-lg">
                          {facility.icon}
                        </div>
                      </div>
                      
                      <div className="p-6 md:p-8 flex-1 flex flex-col">
                        <h3 className="text-xl text-white font-bold mb-3 group-hover:text-[#FFBF00] transition-colors">{facility.title}</h3>
                        <p className="text-gray-400 text-sm leading-relaxed flex-1">{facility.summary}</p>
