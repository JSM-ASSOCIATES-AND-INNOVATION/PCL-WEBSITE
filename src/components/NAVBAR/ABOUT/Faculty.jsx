          ...
          {facultyMembers.map((faculty, idx) => (
            ...
            >
              <Link to={`/about/faculty/${faculty.id}`} className="block group cursor-pointer h-full">
                <div className="bg-[#111] rounded-t-xl overflow-hidden aspect-[3/4] relative border border-white/5 border-b-0">
                  <img 
                    src={faculty.image} 
                    alt={faculty.name} 
                ...
                <div className="bg-[#0a0a0a] border border-white/5 border-t-[#FFBF00] border-t-4 p-6 rounded-b-xl group-hover:bg-[#151515] transition-colors h-[120px] flex flex-col justify-center">
                  <h3 className="text-xl text-white font-bold mb-1 group-hover:text-[#FFBF00] transition-colors">{faculty.name}</h3>
                  <p className="text-gray-400 text-sm mb-1">{faculty.role}</p>
