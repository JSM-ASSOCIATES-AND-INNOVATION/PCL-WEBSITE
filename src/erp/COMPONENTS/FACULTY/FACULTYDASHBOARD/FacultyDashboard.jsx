...
export default function FacultyDashboard({ setActiveTab }) {
        ...
        const fetchDashboardTelemetry = async () => {
            try {
                ...
                const [
                    ...
                    supabase.from('timetable').select('*, subjects(name, code)').eq('faculty_id', userSession.db_id).eq('day_of_week', todayDay).order('start_time', { ascending: true }),
                    ...
                    supabase.from('admin_notices').select('*', { count: 'exact', head: true }).gte('created_at', new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString()),
    ...
    return (
        <div className="w-full max-w-7xl mx-auto flex flex-col gap-8 pb-12 animate-fade-in selection:bg-themeElevated">
            ...
            <div className="w-full bg-themeElevated shadow-2xl rounded-themePanel p-8 relative overflow-hidden border-theme border-themeBorder transition-all duration-300">
                <div className="absolute top-0 right-0 w-96 h-96 bg-themePanel rounded-full blur-3xl -translate-y-1/2 translate-x-1/3 pointer-events-none opacity-50"></div>
                <div className="absolute bottom-0 left-0 w-64 h-64 bg-themePanel rounded-full blur-3xl translate-y-1/2 -translate-x-1/4 pointer-events-none opacity-50"></div>

                <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div>
                        <p className="text-themeAccent font-bold text-xs uppercase tracking-widest mb-2"><i className="fa-regular fa-calendar mr-1"></i> {todayStr}</p>
                        <h1 className={`${theme.text.heading} text-3xl md:text-4xl font-black text-themeText tracking-tight mb-2`}>
                            Welcome back, <span className="text-themeAccent">{userSession?.name?.split(' ')[0] || "Professor"}</span>.
                        </h1>
                    ...
                    <div className="flex flex-col sm:flex-row gap-3 shrink-0">
                        <button onClick={() => setActiveTab('notices')} className="px-8 py-4 bg-themePanel hover:bg-themeElevated border-theme border-themeBorderStrong text-themeTextSec hover:text-themeText rounded-themePanel text-xs font-black uppercase tracking-widest transition-all shadow-lg hover:shadow-xl flex items-center justify-center gap-2 group">
                            <i className="fa-solid fa-satellite-dish text-themeAccent group-hover:scale-110 transition-transform"></i> Announcements
                        </button>
                        <button onClick={() => setActiveTab('roster')} className="bg-themeAccent text-[#0a0a0a] px-8 py-4 rounded-themePanel text-xs font-black uppercase tracking-widest transition-all duration-300 active:scale-95 shadow-lg hover:shadow-xl flex justify-center items-center gap-2 hover:opacity-90 relative overflow-hidden group">
