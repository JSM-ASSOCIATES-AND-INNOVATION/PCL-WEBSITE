...
// --- SHARED STYLES ---
const INPUT_CLS = "w-full bg-themePanel border-theme border-themeBorderStrong rounded-[2rem] px-4 py-3 text-xs lg:text-sm font-bold text-themeText outline-none focus:border-themeAccent transition-all";
        ...
        printWindow.document.write(`
            <html>
                ...
                <body>
                    ...
                    <div class="footer">
                        <div class="sig">HOD Signature<br/>(${req.hod_name})</div>
                        <div class="sig">Registrar / Dean</div>
                    </div>
                    <script>
        ...
        <div className="w-full max-w-7xl mx-auto flex flex-col gap-6 lg:gap-8 pb-32 lg:pb-12 animate-fade-in selection:bg-themeElevated">
                ...
                <div className="flex flex-col gap-8 lg:gap-10 animate-fade-in">
                    ...
                    <div className="flex flex-col gap-6">
                        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 border-b-theme border-black/10 dark:border-white/20 pb-4">
                            ...
                            <button onClick={() => setShowExpModal(true)} className="px-5 py-2.5 bg-themeText hover:bg-themeText/90 text-themePanel rounded-[2rem] text-[10px] lg:text-xs font-black uppercase tracking-widest transition-all shadow-md active:scale-[0.98] flex items-center gap-2">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 lg:gap-6">
                            {experiences.length === 0 ? (
                                <div className="col-span-full py-16 lg:py-24 text-center border-2 border-dashed border-black/10 dark:border-white/20 rounded-[2rem] bg-white/5 backdrop-blur-[80px] shadow-inner/50 backdrop-blur-sm px-4">
                                ...
                                experiences.map((log) => {
                                    ...
                                    return (
                                        <div key={log.id} className={`${theme.layout.panel} rounded-[2rem] border-theme ${isExpanded ? 'border-black/5 dark:border-white/10 shadow-lg shadow-themeAccent/5' : 'border-black/10 dark:border-white/20'} hover:border-black/5 dark:border-white/10 transition-all group flex flex-col`}>
                                            ...
                                            {isExpanded && (
                                                <div className="border-t-theme border-black/10 dark:border-white/20 animate-fade-in bg-themeElevated/20 rounded-b-themePanel">
                                                    <div className="px-5 lg:px-6 py-5">
                                                        ...
                                                        <p className="text-[11px] lg:text-xs text-themeTextSec leading-relaxed bg-themePanel border-theme border-themeBorderStrong p-4 rounded-[2rem] italic shadow-inner">
                                                            "{log.description}"
                                                        </p>
                                                    </div>
                                                    ...
                                                    {log.certificate_notes && (
                                                        <div className="px-5 lg:px-6 pb-5">
                                                            ...
                                                            <p className="text-[11px] lg:text-xs text-themeTextSec leading-relaxed bg-emerald-500/5 p-4 rounded-[2rem] border-theme border-emerald-500/20 shadow-inner">
                                                                {log.certificate_notes}
                                                            </p>
                                                        </div>
                                                    )}
                                                        ...
                                                        {dailyLogs.length === 0 ? (
                                                            ...
                                                            <div className="space-y-3 max-h-56 overflow-y-auto pr-2 custom-scrollbar">
                                                                {dailyLogs.sort((a, b) => new Date(b.date) - new Date(a.date)).map((entry, i) => (
                                                                    <div key={i} className="flex items-start gap-3 bg-themePanel border-theme border-themeBorderStrong p-3.5 rounded-[2rem] shadow-sm hover:border-black/5 dark:border-white/10 transition-colors">
                                                    ...
                                                    <div className="px-5 lg:px-6 pb-6 pt-4 border-t-theme border-black/10 dark:border-white/20 flex flex-col gap-3 bg-themePanel/30">
                                                        <button onClick={(e) => shareToLinkedIn(e, log)} className="w-full py-3 bg-[#0a66c2] hover:bg-[#004182] text-white rounded-[2rem] text-[10px] font-black uppercase tracking-widest transition-all shadow-md active:scale-[0.98] flex items-center justify-center gap-2 group/btn">
                    ...
                    <div className="flex flex-col gap-6">
                        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 border-b-theme border-black/10 dark:border-white/20 pb-4">
                            ...
                            <button onClick={() => setShowPracModal(true)} className="px-5 py-2.5 bg-emerald-500/10 text-emerald-400 hover:bg-emerald-500/20 border-theme border-emerald-500/20 rounded-[2rem] text-[10px] lg:text-xs font-black uppercase tracking-widest transition-all active:scale-[0.98] flex items-center gap-2">
                                <i className="fa-solid fa-plus"></i> Add Practical Log
                            </button>
                        </div>

                        {/* Progress Panel */}
                        <div className={`${theme.layout.panel} p-5 lg:p-6 rounded-[2rem] shadow-sm`}>
                            ...
                            <div className="h-2.5 lg:h-3 w-full bg-themePanel border-theme border-themeBorderStrong rounded-full overflow-hidden border border-black/10 dark:border-white/20 shadow-inner">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 lg:gap-6">
                            {practicalLogs.length === 0 ? (
                                ...
                                practicalLogs.map((log) => (
                                    <div key={log.id} className={`${theme.layout.panel} p-5 lg:p-6 rounded-[2rem] flex flex-col justify-between hover:border-black/5 dark:border-white/10 hover:shadow-lg hover:shadow-themeAccent/5 transition-all group`}>
                                        <div>
                                            <div className="flex justify-between items-start mb-4">
                                                <span className={`px-2.5 py-1 rounded-md text-[8px] lg:text-[9px] font-black uppercase tracking-widest border-theme ${getTypeTheme(log.type)}`}>
                                                    {log.type}
                                                </span>
                                                <span className="text-[10px] lg:text-xs font-black text-themeText bg-themePanel border-theme border-themeBorderStrong px-3 py-1 rounded-md border border-black/10 dark:border-white/20 shadow-sm">{log.hours} Hours</span>
                                            </div>
            ...
            {view === "noc" && (
                <div className="flex flex-col gap-6 lg:gap-8 animate-fade-in">
                    <div className="flex justify-between items-center border-b-theme border-black/10 dark:border-white/20 pb-4 px-2">
                        ...
                        <button onClick={() => setShowNocModal(true)} className="text-[10px] lg:text-xs font-black text-themePanel bg-themeText uppercase tracking-widest flex items-center gap-2 px-5 py-2.5 rounded-[2rem] transition-all active:scale-[0.98] shadow-md hover:bg-themeText/90">
                    ...
                    <div className="flex flex-col gap-4 lg:gap-5">
                        {nocRequests.length === 0 ? (
                            <div className="w-full py-16 lg:py-24 text-center border-2 border-dashed border-black/10 dark:border-white/20 rounded-[2rem] bg-white/5 backdrop-blur-[80px] shadow-inner/50 backdrop-blur-sm px-4">
                            ...
                            nocRequests.map((req) => (
                                <div key={req.id} className={`${theme.layout.panel} p-5 lg:p-6 rounded-[2rem] flex flex-col lg:flex-row lg:items-center justify-between gap-6 hover:border-black/5 dark:border-white/10 hover:shadow-lg hover:shadow-themeAccent/5 transition-all group`}>
                                    <div className="flex-1">
                                        ...
                                        <p className={`text-[10px] lg:text-xs font-bold text-themeTextSec mb-3 bg-themePanel border-theme border-themeBorderStrong px-3 py-1.5 rounded-lg w-fit shadow-sm`}><i className="fa-regular fa-calendar mr-1.5 text-themeAccent"></i> {req.duration}</p>
                                    ...
                                    {/* Approval Pipeline */}
                                    <div className="w-full lg:w-auto bg-themeElevated/50 border border-black/10 dark:border-white/20 rounded-[2rem] p-4 flex items-center justify-between sm:justify-center gap-3 sm:gap-6 shrink-0 shadow-inner">
                                        ...
                                        <div className="flex flex-col items-center gap-2 w-20 text-center">
                                            <div className={`w-8 h-8 lg:w-10 lg:h-10 rounded-full flex items-center justify-center text-[10px] lg:text-xs border-2 transition-colors ${req.status === 'approved'
                                                ? 'bg-emerald-500/10 border-emerald-500 text-emerald-400 shadow-[0_0_10px_rgba(16,185,129,0.2)]'
                                                : req.status === 'pending_hod' ? 'bg-amber-500/10 border-amber-500 text-amber-400 shadow-[0_0_10px_rgba(245,158,11,0.2)]' : 'bg-themePanel border-theme border-themeBorderStrong border-black/5 dark:border-white/10 text-neutral-600'
                                                }`}>
                                    ...
                                    <div className="w-full lg:w-auto shrink-0 flex items-center justify-center">
                                        {req.status === 'approved' ? (
                                            <button onClick={() => handlePrintNoc(req)} className="w-full lg:w-auto bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 border-theme border-emerald-500/20 px-6 py-3.5 rounded-[2rem] text-[10px] font-black uppercase tracking-widest transition-all shadow-sm active:scale-[0.98] flex items-center justify-center gap-2">
            ...
            {showExpModal && (
                <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/80 backdrop-blur-md animate-fade-in" onClick={() => setShowExpModal(false)}>
                    <div className="bg-transparent w-full max-w-xl rounded-t-[2rem] sm:rounded-[2rem] overflow-hidden border border-black/10 dark:border-white/20 flex flex-col max-h-[90vh] shadow-2xl shadow-black" onClick={(e) => e.stopPropagation()}>
                        ...
                        <form onSubmit={handleExpSubmit} className="p-5 lg:p-6 flex flex-col gap-5 overflow-y-auto flex-1 custom-scrollbar">
                            ...
                            <div className="flex items-center gap-3 bg-[#0a66c2]/10 p-4 rounded-[2rem] border-theme border-[#0a66c2]/20 cursor-pointer hover:bg-[#0a66c2]/20 transition-colors shadow-sm" onClick={() => setShareExpOnSubmit(!shareExpOnSubmit)}>
                                <div className={`w-5 h-5 rounded flex items-center justify-center transition-colors ${shareExpOnSubmit ? 'bg-[#0a66c2] text-white shadow-[0_0_8px_rgba(10,102,194,0.5)]' : 'bg-themePanel border-theme border-themeBorderStrong'}`}>
                            ...
                            {submitSuccess ? (
                                <div className="w-full py-4 bg-emerald-500/10 border-theme border-emerald-500/20 text-emerald-400 rounded-[2rem] text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 shadow-[0_0_15px_rgba(16,185,129,0.1)]"><i className="fa-solid fa-check-circle text-lg"></i> Experience Logged</div>
                            ) : (
                                <button type="submit" disabled={isSubmitting} className="w-full py-4 bg-themeText hover:bg-themeText/90 text-themePanel rounded-[2rem] text-[10px] lg:text-xs font-black uppercase tracking-widest transition-all active:scale-[0.98] disabled:opacity-50 shadow-md">{isSubmitting ? "Writing to Ledger..." : "Log Experience"}</button>
                            )}
                        </form>
                    </div>
                </div>
                ...
                <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/80 backdrop-blur-md animate-fade-in" onClick={() => setShowNocModal(false)}>
                    <div className="bg-transparent w-full max-w-lg rounded-t-[2rem] sm:rounded-[2rem] overflow-hidden border border-black/10 dark:border-white/20 flex flex-col max-h-[90vh] shadow-2xl shadow-black" onClick={(e) => e.stopPropagation()}>
                        ...
                        <form onSubmit={handleNocSubmit} className="p-5 lg:p-6 flex flex-col gap-5 overflow-y-auto flex-1 custom-scrollbar">
                            ...
                            {submitSuccess ? (
                                <div className="w-full py-4 bg-emerald-500/10 border-theme border-emerald-500/20 text-emerald-400 rounded-[2rem] text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 shadow-[0_0_15px_rgba(16,185,129,0.1)]"><i className="fa-solid fa-check-circle text-lg"></i> NOC Routed to Mentor</div>
                            ) : (
                                <button type="submit" disabled={isSubmitting} className="w-full py-4 bg-themeText hover:bg-themeText/90 text-themePanel rounded-[2rem] text-[10px] lg:text-xs font-black uppercase tracking-widest transition-all active:scale-[0.98] disabled:opacity-50 shadow-md">{isSubmitting ? "Routing to Mentor..." : "Submit NOC Request"}</button>
                            )}
                        </form>
                    </div>
                </div>
                ...
                <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/80 backdrop-blur-md animate-fade-in" onClick={() => setShowPracModal(false)}>
                    <div className="bg-transparent w-full max-w-lg rounded-t-[2rem] sm:rounded-[2rem] overflow-hidden border border-black/10 dark:border-white/20 flex flex-col max-h-[90vh] shadow-2xl shadow-black" onClick={(e) => e.stopPropagation()}>
                        ...
                        <form onSubmit={handlePracSubmit} className="p-5 lg:p-6 flex flex-col gap-5 overflow-y-auto flex-1 custom-scrollbar">
                            ...
                            {submitSuccess ? (
                                <div className="w-full py-4 bg-emerald-500/10 border-theme border-emerald-500/20 text-emerald-400 rounded-[2rem] text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 shadow-[0_0_15px_rgba(16,185,129,0.1)]"><i className="fa-solid fa-check-circle text-lg"></i> Practical Hours Logged</div>
                            ) : (
                                <button type="submit" disabled={isSubmitting} className="w-full py-4 bg-emerald-500 hover:bg-emerald-400 text-[#050505] rounded-[2rem] text-[10px] lg:text-xs font-black uppercase tracking-widest transition-all active:scale-[0.98] disabled:opacity-50 shadow-md shadow-emerald-500/20">{isSubmitting ? "Logging..." : "Submit Practical Log"}</button>
                            )}
                        </form>
                    </div>
                </div>
            ...
            {showDailyLogModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-fade-in" onClick={() => setShowDailyLogModal(false)}>
                    <div className="bg-transparent w-full max-w-md rounded-[2rem] overflow-hidden border border-black/10 dark:border-white/20 shadow-2xl shadow-black" onClick={(e) => e.stopPropagation()}>
                        ...
                        <form onSubmit={handleDailyLogSubmit} className="p-5 lg:p-6 flex flex-col gap-5">
                            ...
                            {submitSuccess ? (
                                <div className="w-full py-4 bg-emerald-500/10 border-theme border-emerald-500/20 text-emerald-400 rounded-[2rem] text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 shadow-[0_0_15px_rgba(16,185,129,0.1)]"><i className="fa-solid fa-check-circle text-lg"></i> Entry Added</div>
                            ) : (
                                <button type="submit" disabled={isSubmitting} className="w-full py-4 bg-blue-600 hover:bg-blue-500 text-white rounded-[2rem] text-[10px] lg:text-xs font-black uppercase tracking-widest transition-all active:scale-[0.98] disabled:opacity-50 shadow-md shadow-blue-500/20">{isSubmitting ? "Saving..." : "Add Log Entry"}</button>
                            )}
                        </form>
                    </div>
                </div>
