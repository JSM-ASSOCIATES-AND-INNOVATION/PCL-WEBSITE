...
export default function StudentSupportHub() {
    ...
    const tabs = {
        admin: [
            ...
            { id: "fees", label: "Fee Ledger", icon: "fa-indian-rupee-sign" }
        ],
        guidance: [
            { id: "mentorship", label: "Mentorship Hub", icon: "fa-people-arrows" },
    ...
    return (
        <div className="w-full max-w-7xl mx-auto flex flex-col gap-6 lg:gap-8 pb-32 lg:pb-12 animate-fade-in selection:bg-themeElevated">
            {/* ═══ MASTER HUB HEADER ═══ */}
            <div className={`w-full relative overflow-hidden rounded-[2rem] shadow-2xl p-6 lg:p-8 flex flex-col gap-6 border border-themeBorder bg-gradient-to-r from-rose-600 to-rose-900`}>
                ...
                <div className="flex items-center gap-4 lg:gap-5 relative z-10">
                    ...
                    <div>
                        ...
                        <h1 className={`${theme.text.heading} text-2xl lg:text-3xl tracking-tight text-white mb-1 drop-shadow-md`}>Support Center</h1>
                        ...
                        {tabs[activeCategory].map((t) => (
                            <button
                                ...
                                className={`flex-1 lg:flex-none px-5 py-3 rounded-xl text-[10px] lg:text-xs font-black uppercase tracking-widest transition-all duration-300 whitespace-nowrap flex items-center justify-center gap-2 min-w-max ${
                                    activeTab === t.id 
                                    ? 'bg-white text-rose-900 shadow-[0_4px_15px_rgba(0,0,0,0.1)] border border-white scale-100' 
