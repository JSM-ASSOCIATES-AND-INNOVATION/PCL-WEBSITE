...
export default function AdminLeaveManagement({ isHubView = false }) {
    ...
    return (
        <div className={`w-full max-w-[1600px] mx-auto flex flex-col gap-6 animate-fade-in pb-20 lg:pb-8 ${isHubView ? 'bg-transparent text-themeText font-sans' : ''}`}>
                ...
                <div className={`w-full relative overflow-hidden rounded-[2rem] shadow-2xl p-6 lg:p-8 flex flex-col gap-6 border border-white/5 bg-gradient-to-r from-themeAccent to-themeAccent/80`}>
                    ...
                    <div className="flex items-center gap-4 lg:gap-5 relative z-10 mb-2">
                        <div className="w-14 h-14 lg:w-16 lg:h-16 rounded-[1rem] bg-black/20 backdrop-blur-md border border-black/10 dark:border-white/20 flex items-center justify-center shrink-0 shadow-lg">
                            <i className="fa-solid fa-plane-departure text-white text-2xl lg:text-3xl drop-shadow-sm dark:drop-shadow-md"></i>
                        </div>
                        <div>
                            <h1 className={`${theme.text.heading} text-2xl lg:text-3xl tracking-tight text-white mb-1 drop-shadow-sm dark:drop-shadow-md`}>Leave Management</h1>
            ...
            {activeTab !== "review" && (
                <div className={`flex flex-wrap lg:flex-nowrap p-1.5 bg-themeElevated/90 backdrop-blur-2xl shadow-premiumElevated backdrop-blur-md rounded-2xl border border-black/5 dark:border-white/10 relative z-10 gap-1.5 overflow-x-auto no-scrollbar ${!isHubView ? '-mt-10 lg:-mt-12 ml-6 lg:ml-8 w-fit max-w-full' : ''}`}>
                    {tabs.map((tab) => (
                            <button
                                key={tab.id}
                ...
                {activeTab === "review" && (
                    <div className="flex items-center gap-2 relative z-10 pt-2">
                        <button
                            ...
                            className="px-5 py-3 bg-black/20 backdrop-blur-md border border-black/10 dark:border-white/20 hover:bg-white/20 rounded-xl text-[10px] lg:text-xs font-black uppercase tracking-widest text-white transition-colors flex items-center gap-2 shadow-lg"
                        >
