...
export default function AdminLegalAid({ isHubView = false }) {
    ...
    return (
        <div className={`w-full ${isHubView ? 'bg-transparent text-themeText font-sans' : 'max-w-7xl mx-auto flex flex-col gap-6 lg:gap-8 pb-32 lg:pb-12 animate-fade-in selection:bg-themeElevated'}`}>
                ...
                <div className={`w-full relative overflow-hidden rounded-[2rem] shadow-2xl p-6 lg:p-8 flex flex-col gap-6 border border-white/5 bg-gradient-to-r from-themeAccent to-themeAccent/80`}>
                    ...
                    <div className="flex items-center gap-4 lg:gap-5 relative z-10 mb-2">
                        <div className="w-14 h-14 lg:w-16 lg:h-16 rounded-[1rem] bg-black/20 backdrop-blur-md border border-black/10 dark:border-white/20 flex items-center justify-center shrink-0 shadow-lg">
                            <i className="fa-solid fa-hand-holding-hand text-white text-2xl lg:text-3xl drop-shadow-sm dark:drop-shadow-md"></i>
                        </div>
                        <div>
                            <h1 className={`${theme.text.heading} text-2xl lg:text-3xl tracking-tight text-white mb-1 drop-shadow-sm dark:drop-shadow-md`}>Legal Aid Clinic (CLE)</h1>
            ...
            <div className="bg-themePanel/85 backdrop-blur-2xl shadow-premium rounded-themePanel border border-white/5 p-6">
                ...
                {isLoading ? (
                    ...
                    <div className="flex flex-col gap-4">
                        {diaries.map(d => (
                            <div key={d.id} className="bg-themeElevated/90 backdrop-blur-2xl shadow-premiumElevated p-5 rounded-themePanel border border-white/5 flex flex-col md:flex-row md:items-center justify-between gap-4">
                                <div className="flex-1">
                                    ...
                                    <div className="bg-themePanel/85 backdrop-blur-2xl shadow-premium p-3 rounded border border-black/5 dark:border-white/10">
                                ...
                                <div className="flex flex-row md:flex-col gap-2 shrink-0">
                                    ...
                                    {d.status !== 'pending' && (
                                        <button onClick={() => handleStatusUpdate(d.id, 'pending')} className="px-4 py-2 bg-themePanel/85 backdrop-blur-2xl shadow-premium border border-white/5 hover:bg-themeElevated/90 backdrop-blur-2xl shadow-premiumElevated text-themeTextSec font-bold text-xs rounded transition-colors w-full text-center">
                                            Reset Status
                                        </button>
                                    )}
                                </div>
