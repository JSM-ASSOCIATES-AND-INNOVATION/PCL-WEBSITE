        ...
        <div className="w-full max-w-7xl mx-auto flex flex-col gap-8 pb-12 animate-fade-in selection:bg-themeElevated">
                ...
                <div className="relative z-10 w-full lg:w-auto flex-1">
                    <div className="flex items-center gap-4 mb-3 lg:mb-2">
                        ...
                        <div>
                            ...
                            <p className={`text-white/80 text-xs lg:text-sm font-medium`}>Manage advisees, availability, and log official notes.</p>
                        </div>
                    </div>
                </div>

                <div className="relative z-10 w-full lg:w-auto shrink-0">
                    <div className="text-center px-6 lg:px-8 py-3.5 lg:py-4 bg-white/10 backdrop-blur-sm border border-white/20 rounded-themePanel flex lg:flex-col items-center justify-between lg:justify-center">
                        <p className={`text-xl lg:text-3xl font-black text-white`}>{pendingCount}</p>
                        <p className={`text-[10px] font-black uppercase tracking-widest mt-0 lg:mt-1 text-white/90`}>Pending Requests</p>
                    </div>
                </div>
            </div>
            ...
            <div className={`flex flex-wrap p-1.5 gap-1.5 bg-themePanel rounded-themePanel w-full border border-themeBorder sticky top-20 lg:static z-30 shrink-0`}>
                {[
                    ...
                    <button
                        ...
                        className={`flex-auto sm:flex-1 flex items-center justify-center gap-1.5 lg:gap-2 px-3 lg:px-6 py-3 rounded-lg text-[10px] lg:text-xs font-black uppercase tracking-widest transition-all duration-300 whitespace-nowrap ${activeTab === tab.id
                            ? "bg-themeElevated text-themeAccent shadow-sm border border-themeBorderStrong scale-[1.02]"
                    ...
                    <div className="flex flex-col xl:flex-row gap-6 animate-fade-in">
                            ...
                            {grievances.length === 0 ? (
                                ...
                                grievances.map(g => (
                                    <div key={g.id} className="bg-themePanel border border-themeBorder rounded-2xl p-5 flex flex-col gap-4">
                                        ...
                                        <div className="p-3 bg-themeApp/50 rounded-lg text-sm text-themeText italic">"{g.description}"</div>
                                        {g.status === 'pending' && (
                                            <div className="flex gap-2 pt-2">
                                                ...
                                                }} className="flex-1 py-2 bg-themeAccent hover:bg-themeAccent/90 text-white rounded-lg text-xs font-black uppercase tracking-widest transition-all">Mark Resolved</button>
                                            </div>
                                        )}
                                        {g.resolution_notes && (
                        ...
                        <div className="w-full xl:w-[350px] shrink-0">
                            <form onSubmit={handleReportGrievance} className="bg-themePanel border border-themeBorder rounded-2xl p-5 flex flex-col gap-4 sticky top-6">
                                ...
                                <button type="submit" disabled={isSaving} className="w-full py-3 rounded-xl bg-rose-500 hover:bg-rose-600 text-white text-[10px] font-black uppercase tracking-widest transition-all">Submit Report</button>
                            </form>
                        </div>
                    </div>
                )}
            ...
            {showScheduleModal && (
                <div className="fixed inset-0 z-50 flex justify-center items-center p-4">
                    <div className="absolute inset-0 bg-themeApp/80 backdrop-blur-sm" onClick={() => setShowScheduleModal(false)}></div>
                    <div className="relative bg-themePanel border border-themeBorder rounded-2xl w-full max-w-md shadow-2xl p-6 lg:p-8 animate-slide-up overflow-y-auto max-h-[90vh]">
                        ...
                        <form onSubmit={handleScheduleSession} className="flex flex-col gap-4">
                            ...
                            <button
                                ...
                                className={`w-full py-3.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 mt-2
                                    ${isSaving || !scheduleForm.date || !scheduleForm.time || !scheduleForm.topic.trim()
                                        ...
                                        : 'bg-themeAccent hover:bg-themeAccent/90 text-white shadow-lg shadow-themeAccent/20 hover:-translate-y-0.5'
                                    }`}
                            >
                                {isSaving ? (
